...
Implementare la persistenza del gioco "Indovina numero" su file di testo.

## Descrizione

- Implementare una funzione che scriva l elenco dei tentativi fatti dall utente su un file di testo.
- Il programma chiede all utente di inserire il proprio nome ed usa il nome per creare un file di testo con il nome dell utente.ù
...
Versione con streamwriter:

- Lo streamwriter e lo using sono usati per scrivere su file di testo.
- Il vantaggio di usare lo streamwriter e che non e necessario chiudere il file esplicitamente.
- Il file viene chiuso automaticamente quando il blocco using termina.
- Il blocco using puo essere implementato con altre funzionalita come la lettura di file o la connessione a un database.
- In pratica serve in quelle occasioni in cui si vuole garantire che le operazioni su un oggetto vengano eseguite correttamente e che le risorse vengano rilasciate correttamente.
...
Modificare il metodo `StampaTentativi` in questo modo:

```csharp
void StampaTentativi(Dictionary<string, List<int>> tentativiUtenti, string nomeUtente)
{
    Console.WriteLine("Tentativi effettuati: ");
...
Modificare il metodo `ChiediNomeUtente` in questo modo:

```csharp
void ChiediNomeUtente(Dictionary<string, List<int>> tentativiUtenti, ref string nomeUtente)
{
    Console.WriteLine("Inserisci il tuo nome:");
    nomeUtente = Console.ReadLine();
}
...
Modificare il metodo `GiocaAncora` in questo modo:

```csharp
void GiocaAncora(ref string risposta)
{
    Console.WriteLine("Vuoi giocare di nuovo? (s/n)");
...
Modificare il metodo `IndovinaNumero` in questo modo:
...
Modificare il metodo `Main` in questo modo:
...
static void IndovinaNumero(int numeroDaIndovinare, int tentativi, int punteggio, Dictionary<string, List<int>> tentativiUtenti, string nomeUtente)
{
Console.WriteLine("Indovina il numero. Punteggio massimo: 100 punti.");
...
```bash
git add --all
git commit -m "Aggiunta gestione txt Indovina numero txt"
git push -u origin main
```

## Implementazioni future

- Implementare la scrittura dei punteggi migliori classifica
- Implementare i salvataggi delle partite