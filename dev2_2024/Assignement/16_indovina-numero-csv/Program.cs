...
string risposta = "s";
string filePath="PartiteSalvate.csv"; // Percorso del file CSV
...
if (File.Exists(filePath))
{
    ...
    {
        sw.WriteLine("NomeGiocatore,Punteggio,Tentativi");
    }
}
...
SalvaPartitaNelFile(nomeUtente, punteggio, tentativiUtenti[nomeUtente].Count, filePath);
...
// Metodo per salvare i dati nel file CSV
static void SalvaPartitaNelFile(string nomeUtente, int punteggio, int numeroTentativi, string filePath)
{
    using (StreamWriter sw = File.AppendText(filePath))
    {
        sw.WriteLine($"{nomeUtente},{punteggio},{numeroTentativi}");
    }
}